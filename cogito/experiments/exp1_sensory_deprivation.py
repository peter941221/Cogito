...
class SensoryDeprivationExperiment:
    ...
    def _collect_internal_states(
        ...
        """
        states = []
        obs = self.world.get_observation(self.sim.agent_pos)
        ...
        for step in range(num_steps):
            ...
            if not zero_observation:
                new_pos, energy_change, done = self.world.step(
                    self.sim.agent_pos, action
                )
                self.sim.agent_pos = new_pos
                
                # Compute reward
                reward = self.sim.compute_reward(energy_change, done)
                
                # Get next observation
                next_obs = self.world.get_observation(new_pos)
                
                # Let agent observe and learn
                self.agent.observe_result(obs, next_obs, action, reward, done)
    ...
    def compute_apen(self, states: np.ndarray) -> float:
        ...
        return np.mean(apens) if apens else 0.0
    ...
    def run(self) -> ExperimentResult:
        ...
        self.agent = self.sim.agent
        self.world = self.sim.world
        control_untrained_states = self._collect_internal_states(500, zero_observation=True)
